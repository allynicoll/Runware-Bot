...
Compare these two Runware AI models and write 2-3 sentences of plain-English guidance \
for a developer choosing between them. Be specific — mention actual parameter names, \
ranges, or capabilities where relevant. Do not use markdown headers or bullet points.
...
Shared: ${diff.same.length} identical parameters`;
  ...
  const res = await fetchWithTimeout('https://api.runware.ai/v1', {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    console.error('[compare] Runware API error:', res.status);
    throw new Error('Runware API error');
  }

  const data = await res.json();
  if (data?.errors?.length) {
    console.error('[compare] Runware API-level error:', data.errors[0]);
    throw new Error('Runware API error');
  }
...
module.exports = {
  ...
  async execute(interaction) {
    const wait = checkCooldown(interaction.user.id, 'compare');
    if (wait) {
    ...
    if (modelA.id === modelB.id) return interaction.editReply(`❌ Both IDs resolved to the same model (\`${modelA.id}\`).`);
    ...
    try {
      ...
      console.error('[compare] Schema fetch error:', e);
    ...
    if (diff.different.length) embed.addFields({
    ...
    if (diff.onlyA.length) embed.addFields({
    ...
    if (diff.onlyB.length) embed.addFields({
    ...
    // Remove button entirely on expiry — cleaner than a permanently disabled row
